...
people.each { |name,info| puts name unless info["children"].nil? }
# * What are the names of the people who do not have children?
people.each { |name,info| puts name if info["children"].nil? }
...
# is this lowercase m only?
# sets_of_people[0].each_pair {|key, value| puts key if value["company"].downcase.include?("m")}

sets_of_people[0].each_pair {|key, value| puts key if value["company"].downcase.include?("m") || value["company"].downcase.include?("M")}
