...
puts "each username followed by description. If nil print out NA instead."
twitter_data.each do |hash|
  hash.each do |username, data|
    if data["description"]
      puts "#{username}: #{data["description"]}"
    else
      puts "#{username}: NA"
    end
  end
...
twitter_data.each do |hash|
  hash.each do |username, data|
    if data["number of followers"]
      puts "#{username}: #{data["number of followers"]}"
    end
  end
end
...
twitter_data.each do |hash|
  hash.each do |username, data|
    if data["latest tweet"]
      puts "#{username}: #{data["latest tweet"].size}"
    end
  end
end
...
puts "each username followed by the total character count in that user's last 20 tweets"
twitter_data.each do |hash|
  hash.each do |username, data|
    puts username + "used #{data['last twenty tweets'].join.size} characters in their last twenty tweets."
  end
end
...
twitter_data.each do |hash|
  hash.each do |username, data|
    num_followers << hash[username]['number of followers']
  end
hash.each { |username,data| puts username if hash[username]['number of followers'] == num_followers.max }
end
# Which user has the most friends?
puts "which user has the most friends"
num_friends = []
twitter_data.each do |hash|
  hash.each { |username,data| num_friends << hash[username]['number of friends'] }
end
twitter_data.each do |hash|
  hash.each { |username,data| puts username if hash[username]["number of friends"] == num_friends.max }
end
# Which user has the greatest number of tweets?
puts "which user has the most tweets"
number_of_tweets = []
twitter_data.each do |hash|
  hash.each { |username,data| number_of_tweets << hash[username]["number of tweets"] }
end
twitter_data.each do |hash|
  hash.each { |username,data| puts username if hash[username]['number of tweets'] == number_of_tweets.max }
end
# Which users have a description listed in twitter_data?
puts "which users have a description"
twitter_data.each do |hash|
  hash.each { |username,data| puts username unless hash[username]['description'] == nil }
end
# Which users have a location listed in twitter_data?
puts "which users have a location"
twitter_data.each do |hash|
  hash.each { |username,data| puts username unless hash[username]['location'] == nil }
end
